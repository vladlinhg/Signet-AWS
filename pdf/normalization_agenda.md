# Normalization Agenda: Database Schema Reference

This document acts as the working agenda for planning the PDF Text Normalization logic.
It contains the full database schema to serve as the target for our data mapping.
...
| Field Name | Type | Options / Constraints | Description |
| :--- | :--- | :--- | :--- |
| `code` | CharField(3) | `unique=True` | e.g. `YVR`. |
| `name` | CharField(100)| | |
...
| `username` | CharField | | |
| `email` | EmailField | | |
| `role` | CharField | Choices: `IT_ADMIN`, `MANAGER`, `ACCOUNTANT`, `SALES`, `MARKETING` | |
| `department` | CharField | | |
| `agent_profile` | ForeignKey | `Agent` | Link to external agent profile if applicable. |
